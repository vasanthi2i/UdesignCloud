...
import { clickNextButton } from "../src/Utils/ElementUtil";


test("Verify uassist order can be created for a new case", async ({page}) => {
    //test.slow();
    ...
    new OrdertypePage(page), new Prescriptionpage(page),new SummaryPage(page), 
    new UassistOrderConfirmationPage(page), new TemplatePage(page));
    const[login_page,scans_page,addnewpatient_page,patientlist_page,photographs_page,xrays_page,
        ordertype_page,prescription_page,summary_page,uassist_order_page,
        template_page] = pages;
